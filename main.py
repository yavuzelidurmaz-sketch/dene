import requests
import json
import time

# --- TOKEN (YENİSİNİ ALIP YAPIŞTIR) ---
MANUAL_TOKEN = "eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9..ci3CbqGQHVgUFIPs2PH_tR7CUTzN4HoKu3LY3zpFQztXlqZVgo_kXqp9A-6Pdn0G_R_BDtNC-sWS9eRzgka0KzlP228BGmZ87N_0wpxg1riHierd5LKIMZFNOJw-LkdQ3sFTWhGvD0zJm-lYYunh2gxtoWJXGVyuQYQSlt4xrPEMneUDbw-d0D2nVeJu_WVfkOPMFEC6bEmuFVIHgD6usMkd2_e9sr7mkt7GXwVBGuFJb9dK1p1nWb-KKXN7oIvf-eaxCbtAJ27Lja_NI-YlA8QjvwVsqnmf7qNuJpjJtorPSDvUcR6gp8oiZmzCw8zwJXoB79Xkmxlr0jnxDrTtIQ"

# --- SABİTLER ---
PROJECT_ID = "2da7kf8jf"
PROFILE_ID = "URCMQDLDLXJLHLPFBAN0ZI9V" 

# Senin bulduğun Kategori URL'si
CATEGORY_URL = f"https://api.gain.tv/{PROJECT_ID}/CALL/ProfileTitle/getPlaylistsByCategory/{PROFILE_ID}?slug=%2Ffilm&__culture=tr-tr"

HEADERS = {
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
    "Content-Type": "application/json",
    "Accept": "application/json",
    "x-gain-platform": "web",
    "Origin": "https://www.gain.tv",
    "Referer": "https://www.gain.tv/"
}

def debug_category_list():
    """Hatanın tam sebebini görmek için detaylı istek atar"""
    auth_headers = HEADERS.copy()
    auth_headers["Authorization"] = f"Bearer {MANUAL_TOKEN}"
    
    print(f"🌍 Kategori listesi isteniyor...")
    print(f"📡 URL: {CATEGORY_URL}")
    
    try:
        response = requests.get(CATEGORY_URL, headers=auth_headers)
        
        print(f"\n--- SUNUCU YANITI ---")
        print(f"Durum Kodu: {response.status_code}")
        
        # Cevabı metin olarak alalım
        content = response.text
        
        # Eğer cevap çok uzunsa başını ve sonunu gösterelim
        if len(content) > 2000:
            print(f"İçerik (İlk 1000 karakter):\n{content[:1000]}...")
        else:
            print(f"İçerik (Tamamı):\n{content}")
            
        print("\n---------------------")

        if response.status_code == 200:
            try:
                data = response.json()
                if data.get("Success"):
                    widgets = data.get("Result", {}).get("Widgets", [])
                    print(f"✅ BAŞARILI! {len(widgets)} adet kategori bulundu.")
                else:
                    print("⚠️ JSON geldi ama 'Success' false döndü.")
            except:
                print("⚠️ Cevap 200 OK ama JSON formatında değil!")
        
    except Exception as e:
        print(f"🔥 Bağlantı Hatası: {e}")

def main():
    if "BURAYA" in MANUAL_TOKEN:
        print("⛔ Lütfen Token'ı girmeyi unutma!")
        return

    debug_category_list()

if __name__ == "__main__":
    main()
